...
class Client(system: ActorSystem) extends Actor {
  ...
  def receive = {
    case RegisterUser(userId: Int) => {
      ...
      var genderJson = Random.shuffle(gender.toList).head
      var name = Random.alphanumeric.take(Random.nextInt(50)).mkString
    ...
    case pagePost(pageId: Int) => {
      ...
      var post = Random.alphanumeric.take(Random.nextInt(140)).mkString
       val pipeline: HttpRequest => Future[HttpResponse] = sendReceive
